...
import { Activity, Shield, AlertTriangle, CheckCircle2, Info, RefreshCw } from "lucide-react";
...
export default function DiseasePrediction() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setIsScanning(true);
            setTimeout(() => {
                setRisks(prev => prev.map(group => ({
                    ...group,
                    diseases: group.diseases.map(d => ({
                        ...d,
                        risk: Math.min(100, Math.max(0, d.risk + (Math.random() > 0.5 ? 1 : -1)))
                    }))
                })));
                setIsScanning(false);
            }, 1500);
    ...
    return (
        <section id="predictor" className="py-24 px-6 bg-teal-50/50 relative">
                    ...
                    <AnimatePresence>
                        {isScanning && (
                            <motion.div
                                ...
                                exit={{ opacity: 0 }}
                                className="absolute -top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-2 rounded-full bg-teal-600 text-white text-[10px] font-bold uppercase tracking-widest shadow-lg"
                            >
                                <RefreshCw size={12} className="animate-spin" />
                                Re-scanning Active Bio-data
                            </motion.div>
                        )}
                    </AnimatePresence>
                            ...
                            <div className="space-y-4">
                                {group.diseases.map((disease, dIdx) => (
                                    <motion.div
                                        ...
                                        className="p-6 rounded-3xl bg-white border border-teal-100 shadow-sm hover:shadow-xl hover:shadow-teal-500/5 transition-all group relative overflow-hidden"
                                    >
                                        {isScanning && (
                                            <motion.div
                                                initial={{ x: "-100%" }}
                                        ...
                                        <div className="flex justify-between items-start mb-6">
                                            ...
                                            <div className="text-right">
                                                <div className="text-2xl font-black text-teal-900">{disease.risk.toFixed(0)}%</div>
